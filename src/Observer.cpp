	  ...
	  save_interval_(save_interval), use_periodic_(use_periodic)
{
	...
	log_info("    output velocity file      : {}", pos_filename_);
}
...
void DCDObserver::write_dcd_velocity(std::ofstream& ofs, const OpenMM::State& state)
{
    ...
    // write velocity
    {
        ...
        {
            this->buffer_vel_x_[idx] = static_cast<float>(vel_in_nmps[idx][0]*OpenMM::AngstromsPerNm);
            this->buffer_vel_y_[idx] = static_cast<float>(vel_in_nmps[idx][1]*OpenMM::AngstromsPerNm);
            this->buffer_vel_z_[idx] = static_cast<float>(vel_in_nmps[idx][2]*OpenMM::AngstromsPerNm);
        }
...
void EnergyObserver::write_energy(std::ofstream& ofs, int frame_num,
	const OpenMM::Context& context) const
{
	ofs << std::setw(10) << std::left << frame_num;
	for (auto ffname_groupid : ffname_groupid_map_)
	{
